...
namespace UnityEngine
{
    ...
    public class RuleTile : TileBase
    {
                ...
                if (RuleMatches(rule, ref neighboringTiles, ref transform))
                {
                    switch (rule.m_Output)
                    {
                        ...
                        case TilingRule.OutputSprite.Random:
                            var index = Mathf.Clamp(Mathf.FloorToInt(GetPerlinValue(position, rule.m_PerlinScale, 100000f) * rule.m_Sprites.Length), 0,
                    ...
                    m_GameObjectQuaternion = Quaternion.LookRotation(new Vector3(transform.m02, transform.m12, transform.m22),
        ...
        public virtual void RuleOnGUI(Rect rect, int arrowIndex, int neighbor)
        {
            ...
            var allConsts = m_NeighborType.GetFields(BindingFlags.Public | BindingFlags.Static | BindingFlags.FlattenHierarchy);
