...
namespace UnityEngine
{
    ...
    public class HexagonalRuleTile : RuleTile
    {
        ...
        {
            {
                new Vector3Int(1, 0, 0), new Vector3Int(0, -1, 0), new Vector3Int(-1, -1, 0), new Vector3Int(-1, 0, 0), new Vector3Int(-1, 1, 0),
                new Vector3Int(0, 1, 0)
            },
            {
                new Vector3Int(1, 0, 0), new Vector3Int(1, -1, 0), new Vector3Int(0, -1, 0), new Vector3Int(-1, 0, 0), new Vector3Int(0, 1, 0),
        ...
        protected override bool RuleMatches(TilingRule rule, ref TileBase[] neighboringTiles, ref Matrix4x4 transform)
        {
            ...
            for (var angle = 0; angle <= (rule.m_RuleTransform == TilingRule.Transform.Rotated ? 300 : 0); angle += 60)
            {
        ...
        protected override Matrix4x4 ApplyRandomTransform(TilingRule.Transform type, Matrix4x4 original, float perlinScale, Vector3Int position)
        {
            ...
            switch (type)
            {
                ...
                case TilingRule.Transform.Rotated:
                    var angle = Mathf.Clamp(Mathf.FloorToInt(perlin * neighborCount), 0, neighborCount - 1) * (360 / neighborCount);
        ...
        protected override void GetMatchingNeighboringTiles(ITilemap tilemap, Vector3Int position, ref TileBase[] neighboringTiles)
        {
            ...
            {
                var tilePosition = position + GetOffsetPosition(position, index);
